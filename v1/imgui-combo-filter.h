...
enum ImGuiComboInputTextFlags_
{
    ...
    ImGuiComboInputTextFlags_HeightMask_             = ImGuiComboInputTextFlags_HeightSmall | ImGuiComboInputTextFlags_HeightRegular | ImGuiComboInputTextFlags_HeightLarge | ImGuiComboInputTextFlags_HeightLargest,
...
bool ComboAutoSelect(const char* combo_label, char* input_text, int input_capacity, int& selected_item, const T1& items, ComboItemGetterCallback<T2> item_getter, ComboAutoSelectSearchCallback<T2> autoselect_callback, ImGuiComboInputTextFlags flags = ImGuiComboInputTextFlags_None);
template<typename T1, typename T2, typename = std::enable_if<std::is_convertible<T1, T2>::value>::type>
bool ComboAutoSelect(const char* combo_label, char* input_text, int input_capacity, int& selected_item, const T1& items, ComboItemGetterCallback<T2> item_getter, ImGuiComboInputTextFlags flags = ImGuiComboInputTextFlags_None);
...
template<typename T1, typename T2, typename = std::enable_if<std::is_convertible<T1, T2>::value>::type>
bool ComboAutoSelectEX(const char* combo_label, char* input_text, int input_capacity, int& selected_item, const T1& items, ComboItemGetterCallback<T2> item_getter, ComboAutoSelectSearchCallback<T2> autoselect_callback, ImGuiComboInputTextFlags flags);
template<typename T1, typename T2, typename = std::enable_if<std::is_convertible<T1, T2>::value>::type>
bool ComboFilterEX(const char* combo_label, char* input_text, int input_capacity, int& preview_item, int& selected_item, ComboFilterSearchResults& filtered_items, const T1& items, ComboItemGetterCallback<T2> item_getter, ComboFilterSearchCallback<T2> filter_callback, ImGuiComboInputTextFlags flags);
...
bool ComboAutoSelect(const char* combo_label, char* input_text, int input_capacity, int& selected_item, const T1& items, ComboItemGetterCallback<T2> item_getter, ComboAutoSelectSearchCallback<T2> autoselect_callback, ImGuiComboInputTextFlags flags)
{
...
template<typename T1, typename T2, typename>
bool ComboAutoSelect(const char* combo_label, char* input_text, int input_capacity, int& selected_item, const T1& items, ComboItemGetterCallback<T2> item_getter, ImGuiComboInputTextFlags flags)
{
...
bool ComboAutoSelectEX(const char* combo_label, char* input_text, int input_capacity, int& selected_item, const T1& items, ComboItemGetterCallback<T2> item_getter, ComboAutoSelectSearchCallback<T2> autoselect_callback, ImGuiComboInputTextFlags flags)
{
	...
	const float w = (flags & ImGuiComboInputTextFlags_NoPreview) ? arrow_size : expected_w;
	...
	if (!popupIsAlreadyOpened) {
		if (pressed) {
			...
			if (!(flags & ImGuiComboInputTextFlags_NoPreview))
				window->DrawList->AddRectFilled(frame_bb.Min, ImVec2(value_x2, frame_bb.Max.y), frame_col, style.FrameRounding, (flags & ImGuiComboInputTextFlags_NoArrowButton) ? ImDrawFlags_RoundCornersAll : ImDrawFlags_RoundCornersLeft);
		}
	}
	...
	if (!popupIsAlreadyOpened) {
		...
		if (input_text != NULL && !(flags & ImGuiComboInputTextFlags_NoPreview)) {
			RenderTextClipped(
	...
	if (!(g.NextWindowData.Flags & ImGuiNextWindowDataFlags_HasSizeConstraint)) {
		...
		if      (flags & ImGuiComboInputTextFlags_HeightRegular) popup_item_count = 8;
		else if (flags & ImGuiComboInputTextFlags_HeightSmall)   popup_item_count = 4;
		else if (flags & ImGuiComboInputTextFlags_HeightLarge)   popup_item_count = 20;
...
template<typename T1, typename T2, typename>
bool ComboFilterEX(const char* combo_label, char* input_text, int input_capacity, int& preview_item, int& selected_item, std::vector<ComboFilterResultData>& filtered_items, const T1& items, ComboItemGetterCallback<T2> item_getter, ComboFilterSearchCallback<T2> filter_callback, ImGuiComboInputTextFlags flags)
{
	ImGuiContext* g = GImGui;
	ImGuiWindow* window = GetCurrentWindow();
	...
	if (preview_string != NULL && !(flags & ImGuiComboInputTextFlags_NoPreview)) {
...
struct ComboAutoSelectData
{
	...
	{
		strncpy(_InputBuffer, _ItemGetter(GetItem(), _SelectedItem), BufferSize - 1);
	}
...
template<typename T1, typename T2>
bool ComboAutoSelect(const char* combo_label, ComboAutoSelectData<T1, T2>& combo_data, ImGuiComboInputTextFlags flags = ImGuiComboInputTextFlags_None)
{
	return ComboAutoSelect(combo_label, combo_data._InputBuffer, combo_data.BufferSize, combo_data._SelectedItem, combo_data.GetItem(), combo_data._ItemGetter, combo_data._ItemSearcher, flags);
}
...
template<typename T1, typename T2>
bool ComboFilter(const char* combo_label, ComboFilterData<T1, T2>& combo_data, ImGuiComboInputTextFlags flags = ImGuiComboInputTextFlags_None)
{
	BeginDisabled(Internal::IsContainerEmpty(combo_data.GetItem()));
	auto ret = Internal::ComboFilterEX(combo_label, combo_data._InputBuffer, combo_data.BufferSize, combo_data._PreviewItem, combo_data._SelectedItem, combo_data._FilteredItems, combo_data.GetItem(), combo_data._ItemGetter, combo_data._ItemSearcher, flags);
	EndDisabled();
